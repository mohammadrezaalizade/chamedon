...
const Slider: React.FC<Props> = ({ visibility, setVisibility }) => {
  ...
  return (
    <AnimatePresence key="slider" >
      {visibility && (
        <aside className="absolute z-[90] w-screen h-screen  overflow-hidden select-none">
          <motion.div
            variants={slider}
            initial="hidden"
            animate="show"
                        ...
                        <motion.a
                          ...
                          className={
                            router.pathname === navigator.path
                              ? "bg-gradient-to-tl from-[#0094ff] to-[#87ccfd] text-[#f1f1f1] p-2 rounded-md shadow-md"
                              : "bg-gradient-to-tl p-2 rounded-md hover:from-[#e2e2e2] hover:to-[#ccc] transition ease-in-out duration-200 bg-transparent hover:text-[#313131]"
                          }
                        >
                          {navigator.title}
                        </motion.a>
